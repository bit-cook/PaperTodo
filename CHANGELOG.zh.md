...
- **插件读写与失败处理精简**：查询纸片、待办、笔记和图片不再提交其他正文，外部写入也不再全局 Commit 无关插件正文；如果写入目标就是同一张内置 Markdown，会先应用这张纸片尚未写回模型的用户文字，核心保存只同步内置 Markdown。插件状态只使用普通数据文件，读取失败直接报告，不再自动从空状态创建恢复文件；旧恢复文件不会被自动删除。普通主题、字号或激活通知出错不再撤掉整个插件正文；后台首次启动失败不再自动重试。
- **插件交互与待办创建**：新建或追加待办时，初始完成状态、提醒和关联不再额外要求修改已有内容的权限。按钮和顶部标签的提示支持换行，小弹窗里的网页/邮件链接交给系统默认程序打开。复盘记录池不再依赖独立备份文件完成保存。

- 缩短纸片标题长度上限后，待办中关联纸片的名称会立即同步更新。

- **跨纸片显示控制**：MCP 和 Native/Web 插件可按 ID 显示、隐藏、展开、折叠或激活已有纸片。显示保留折叠状态，隐藏不删除内容；MCP 查询同时返回纸片折叠状态。

- **插件 API 2.2**：插件协议升级到 2.2，同时继续兼容 2.1 插件。`apiVersion` 明确定义为插件最低要求的宿主 API：宿主接受“最低兼容版本～当前版本”范围内的插件，低于最低版本或高于当前宿主版本都拒绝加载；`hidden` 启动纸片、设置页 `action` 按钮以及公共软件 Settings API/权限明确归入 2.2 合同。

- **软件设置 API**：MCP 与 Native/Web 插件可通过同一类型化目录查询、修改面向用户的设置，支持保存失败回滚；MCP 同时补齐待办解绑。

- **插件设置动作**：插件可声明由自身 Runtime 处理的自定义按钮，例如立即刷新、测试连接，无需配置快捷键；动作不可用时按钮禁用。修复启动恢复延迟执行时可能再次隐藏刚打开的 Codex 提示词纸片。
- 修复关闭「展开时保留边缘胶囊」后，贴边胶囊无法正确记住展开纸片位置和大小的问题。
- **外部打开回退**：外部打开笔记仍优先使用当前配置导出后缀对应的系统默认应用；无论改成哪种后缀，只要 Windows 没有关联应用，就会自动改用记事本打开。
- **窗口焦点**：关闭、隐藏或删除前台纸片后，正常回到其后方的可用窗口，不再把被遮挡的后台窗口突然拉到前面；处理后台纸片不会主动抢焦点。

- **纸片背景缩放修正**：“拉伸”现在会保持图片宽高比并铺满纸片；关闭“拉伸”时，小图保持原始尺寸，大图仅按比例缩小到完整放进纸片，不会被放大或直接裁切。
- **Codex CLI Bridge**：默认模型更新为 GPT-6 Astra（`gpt-6-astra`）；推理档位仍为 `xhigh`，模型设置留空时仍不覆盖 Codex CLI 自己的模型配置。发送任务默认改为前台打开 PowerShell，仍可选择后台静默执行；插件界面与默认提示词会跟随 PaperTodo 当前语言，本地化切换不会覆盖用户已经修改过的默认提示词。插件设置改用独立的**更多设置**页：启动、后台执行和模型保留在插件卡片上，推理档位与路径选项继续保留在完整设置页中。新增默认开启且互相独立的“开启插件技能”“开启本软件操作技能”“允许本插件开启 MCP”；技能改为发送时注入，默认提示词保持独立。任务需要 MCP 时，插件可自动开启 MCP 总开关及全部写入、删除权限，但不会改动普通 PaperTodo 功能开关。默认提示词还会要求待办保持简短，长内容仅在“待办关联纸片”已开启时拆到绑定 Note；笔记避免过度冗长，但也不做无意义的过度精简。Codex 提示词编辑纸片改为启动时默认隐藏，并在插件“更多设置”提供“编辑提示词”按钮直接展开打开。MCP 按需开启命令会通过退出码返回宿主的实际执行结果，不再把“命令已发送”当作“已经开启”。
- **官网交互演示更新**：官网通过同一套工作台展示 PaperTodo 4.0 的纸片、边缘浏览、Markdown、脚本与插件制作；同步更新常见问题，支持纸张配色、深色模式，并在 1600 × 900 以上等比放大整页。首次访问跟随浏览器语言显示中文或英文，链接指定和已保存的语言选择优先。

#### 优化和 bug 修复

- 修复 Web Mini 恢复显示后，网页内容正常但部分按钮可能无法点击的问题；恢复完成前会重新同步当前交互区域，普通布局变化仍保留去重，不增加常驻消息量。
- 修复多条待办提醒同时到期、托盘通知不可用时，可能只实际打开第一条却把整批都标记为已提醒的问题；现在只有真正触达的提醒会被确认，其余继续按原有机制重试。
- 修改“优先 PowerShell 7”或“隐藏运行窗口”只影响后续脚本执行，不再因为调整这两个偏好而停止当前常驻 PowerShell 进程；显式关闭常驻进程功能时仍会正常停止。
- 暂时关闭胶囊、边缘胶囊或主胶囊相关功能时，不再清除已经记住的队列高度位置；重新开启后可继续使用原来的布局，恢复默认仍会清除该记忆。
- 复盘记录池读取已有数据失败、数据损坏或版本不受支持时会直接报告错误，不再按空记录继续运行并在后续保存时覆盖原文件。
- 更新 WebView2（1.0.4191.47）、Microsoft.Extensions.Hosting（10.0.12）和 MCP SDK（2.2.0），跟进运行时稳定性与兼容性修复。
...
- **自定义纸片背景**：在 `PaperTodo.exe` 旁放入 `papertodo.png`、`papertodo.jpg` 或 `papertodo.jpeg`，可自定义纸片背景。
...
- **本地数据优先**：支持基础的待办管理与便签创建，数据采用本地 JSON 格式存储，完全离线运行。
