...
- **边缘胶囊收缩优化**：标题限制长度时，可收缩范围增加，可完全收缩。
- 待办与笔记胶囊的图标和标题对齐。
- 隐藏关闭按钮不留空位。
- 主胶囊精简，仅显示数字。
...
- **便签微应用容器**：笔记纸可按需切换为独立的桌面微应用（如专注番茄钟、拟物时钟、复盘池或系统监控面板）；设置中新增独立「插件」管理中心，数据安全隔离保存在 `plugins/data/` 目录下（10 MB / 20 MB 限制）。
  ...
  - 示例：
    - 示例插件：Codex CLI 桥接（可将待办/纸片上下文发送到本地 Codex CLI）、原生时钟、专注番茄钟、复盘记录池、Web 时钟等，适配窄窗口、胶囊与 Mini 视图。
...
- **Markdown 所见即所得编辑**：新增完全渲染，达到真正MD完全渲染体验，支持块渲染，编辑时也可以完全渲染，不会跳变，体验优于大部分Md编辑软件。
...
- **自定义纸片背景**：在 `PaperTodo.exe` 旁放入 `papertodo.png`、`papertodo.jpg` 或 `papertodo.jpeg`，可自定义纸片背景。
  - 视觉设置可选原图或叠加纸片配色，支持拉伸 / 居中 / 左下 / 正下 / 右下。
  - 解码最长边为 4096 像素。
- **设置重构**：左侧导航，并优化显示；按当前工作区适配小屏、高缩放与跨 DPI 切屏；开关和快捷键录制局部刷新，减少闪动。
