...
[English](doc/CHANGELOG.en.md) · [中文首页](README.zh-CN.md)
...
- **启动与退出响应**：显示器尚未就绪时，只延后相关纸片，其他胶囊和纸片可先使用；边缘笔记先准备可浏览的预览，再按低优先级短批次补建折叠纸片，右键菜单和可选初始化不阻挡首轮恢复；插件启动纸片不再轮询等待。正常退出保留最后一次保存，随后先撤下界面，同时停止脚本进程，并完成正常窗口退出流程，减少逐个等待和进程结束拖延。

**边缘预览卡片**

- **边缘胶囊实时预览**：鼠标悬停在边缘胶囊上即可展开轻量、可交互的预览卡片，无需打开完整纸片即可即时浏览内容。
  - **待办预览**：实时显示简化待办列表，支持直接勾选完成或撤销，点击卡片背景可直接呼出完整纸片；关联或快启操作使用更稳定的独立点击区域，并按实际可用文字宽度排版，减少短文本被右侧操作挤换行。卡片只展示当前空间内的顶部内容，不支持滚动，溢出时在底部显示省略号。
  - **笔记预览**：跟随 Markdown 渲染设置，关闭时显示原文，基础保留标记并提供轻量高亮，完全渲染隐藏语法标记并显示列表圆点、分割线，编号和任务状态保持清晰；支持标题、强调、删除线、代码块、列表与图片占位。文字沿用正文的字体、字号、加粗、文字渲染设置及单篇缩放，普通行和空行不额外加宽间距。卡片只展示当前空间内的顶部内容，不支持滚动，溢出时在底部显示省略号；长笔记最多准备前 16 个内容块、6000 字符，卡片尺寸与显示使用同一份预览内容，不为预算外内容预留空白；普通短行、长段落和代码块统一按可见空间在后台排版，减少对界面响应的占用及不必要的文字对象分配；首次打开时正文会在第一批内容准备完成后一次显示，避免半成品内容闪现；相同内容和尺寸的预览在短暂收起后恢复时复用已完成结果，内容、字体、缩放或屏幕变化后重新准备。行内语法沿用正文的识别实现，改善嵌套强调、转义与复杂链接的一致性。启动恢复和内容变化后，会提前排好边缘笔记预览，反复切换时复用结果、减少首次浏览等待，不额外保留隐藏卡片；开启边缘浏览且合格笔记不超过 10 张时，短笔记也全部预热，更多笔记时优先准备较长或样式较多的内容。启动首批不额外等待编辑合并延迟，连续编辑仍合并处理，离开边缘队列后释放对应缓存。预热与正式显示使用一致的正文尺寸；长段落链接遵循按下、释放的完整点击判定。
  - **流畅接续与意图预测**：内置可调节的鼠标移动意图预测，在胶囊间连续浏览时减少误切换；展开、收回时卡片与相邻胶囊同时运动，减少动画启动时的错位；优化连续浏览时的动画更新节奏，改善展开、收回和切换的更新连续性；单个队列更新不再暂停其他队列；默认向下切换时，只要目标卡片仍能完整显示，就优先保持当前鼠标附近的展开位置，不会为了填补上方空位突然跳动；常规设置可关闭「浏览时优先向下展开」，恢复优先利用上方空位的原补位规则；离开可浏览区域后自动收回；隐藏或恢复边缘队列时，正确结束旧的悬停状态；动画期间的鼠标按下保留原始位置和按键状态，仅在当次交接立即成功时转交，交接需要重试时不会迟到重放旧点击。
  - **统一尺寸与拖拽保护**：拖动胶囊排序或移出时自动收起预览；空待办与空笔记默认以更紧凑的卡片尺寸展示。
- **边缘标题长度**：支持隐藏标题文字（0）或无限制，加减可循环切换；关闭悬停预览时，被截短的标题会随胶囊悬停展开，离开后收回，原始标题不受影响。
- **边缘胶囊重复点击找回 / 收回**：展开纸片仍明显可见时，再次点击对应边缘胶囊会将其收回；如果纸片被其他窗口明显遮挡，则优先带回前台而不是在看不见的位置折叠。
- **胶囊尺寸与计数优化**：待办与笔记边缘胶囊统一图标槽和标题起点，隐藏关闭按钮时不再保留空白占位；主胶囊展开或收起时都始终显示准确数量，1～99 保持紧凑的两位数固定宽度，100+ 按实际数量扩展，状态切换时不再因数字和箭头变化左右伸缩。
...
**待办与 Markdown**
- **内置全文搜索**：`Ctrl+F` 可跨全部待办和 Markdown 笔记查找，包含已完成的待办，显示当前纸片与全局命中计数；`Enter` / `Shift+Enter` 或箭头按钮可循环跳转并定位到具体待办项或笔记文字，自动显示隐藏纸片、展开胶囊并接续搜索。搜索框保持输入焦点时，当前待办或笔记命中仍会保持可见高亮。搜索框唤出后可直接输入，从纸片内展开并允许超出边框；可通过靠右的圆角竖条拖动柄、计数区或空白区域移动，采用无阴影样式，上下箭头和关闭按钮使用矢量图标。
- **自定义笔记背景**：在 `custom/note/` 中放入 `background.png`、`background.jpg` 或 `background.jpeg`，即可为内置 Markdown 笔记使用自定义背景；检测到背景图片后，「外观」设置中会自动显示启用开关，并随浅色 / 深色主题调整显示效果。关闭状态会被记住，替换背景图片后可重新切换开关或重启 PaperTodo 刷新。
- **完全渲染升级为所见即所得块级编辑**：Markdown 显示简化为「关闭 / 基础 / 完全渲染」；旧基础保持不变，旧增强与完全统一为完全，旧配置自动迁移，新安装及恢复默认选择完全。笔记选择「完全渲染」档，编辑时也按标题、列表、引用、代码块、图片与行内样式等最终排版直接显示，并隐藏多数 Markdown 标记
标题、加粗、链接等文字紧凑重排；任务框、无序列表和引用使用稳定占位，减少勾选、编辑切换与引用续行时的横向跳动；完全渲染中的任务复选框可直接点击勾选或取消，实际写回原始 Markdown `[ ]` / `[x]` 并正常进入撤销 / 重做历史；有序列表编号始终直接显示原文字，避免编辑切换时闪动；光标所在块会显示其源码标记，失焦回到整篇只读渲染。
...
- **待办连续多选與批量操作**：在待办项左侧区域滑动即可连续框选多条事项，支持批量勾选完成/恢复、批量复制、右键删除全部或拖拽整组丢弃。
...
- **失焦形态与置顶自定义**：支持纸片失焦时仅隐藏顶栏按钮，或淡出标题栏并保留完整的圆角、描边和阴影，完全隐藏后可点击到后面的窗口，正文位置和窗口尺寸保持不变；支持让贴边胶囊与主胶囊不再强制保持系统最前台置顶。
...
- **安装包体积优化**：不含 .NET 运行时的 Windows 单文件包由约 33 MiB 缩小至约 17 MiB；测试中未观察到明显的启动或内存回退。
