...
- **Avalonia 12 + .NET 10 全面迁移**：先完成 Windows 10/11 版本并启用 Native AOT，保持现有 `data.json`、图片库和核心交互兼容；贴边胶囊改为每个显示器边缘共用一个固定合成面，悬停拉出、浏览切换和队列补位只更新内部合成节点，不再逐帧移动或缩放系统窗口。跨平台发布在 Windows 版本稳定后继续推进。

- **插件迁移**：官方原生能力改为随程序静态编译，继续支持运行时 Web 插件；旧 WPF 原生插件的数据和 provider 标识必须保留并明确提示不兼容，不得在迁移时静默清空。后续第三方原生扩展评估独立进程协议。
