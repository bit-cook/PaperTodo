    ...
    private static readonly IReadOnlyDictionary<string, TextSet> TextByLanguage =
        new Dictionary<string, TextSet>(StringComparer.OrdinalIgnoreCase)
        {
            ["zh"] = new(
                ...
                "顶栏显示新建 Todo", "顶栏显示新建 Note", "顶栏显示外部编辑", "主题", "配色", "整体缩放", "文字", "Todo 字号", "Note 字号",
                "标题字号", "胶囊字号", "粗体", "窗口透明度", "非活动纸片透明", "非活动透明度", "区分小键盘数字",
                "只注册 Avalonia 当前已经实现的通用快捷键；贴边和实验室快捷键保留原配置，不会被此页覆盖。", "快捷键无效",
                "Todo 提醒", "快速提醒（分钟）", "Markdown 模式", "外部 Markdown 后缀", "用外部编辑器打开",
            ...
            ["en"] = new(
                ...
                "Show New Todo on top bar", "Show New Note on top bar", "Show external editor on top bar", "Theme", "Color scheme", "Overall scale", "Text", "Todo size", "Note size",
                ...
                "Todo reminders", "Quick reminder (minutes)", "Markdown mode", "External Markdown extension", "Open in external editor",
            ...
            ["ja"] = new(
                ...
                "トップバーに新規 Todo", "トップバーに新規 Note", "トップバーに外部エディター", "テーマ", "配色", "全体スケール", "文字", "Todo サイズ", "Note サイズ",
                "タイトルサイズ", "カプセルサイズ", "太字", "ウィンドウ透明度", "非アクティブ付箋を薄く", "非アクティブ透明度", "テンキー数字を区別",
                "Avalonia で実装済みの一般ショートカットのみ編集します。エッジ/Labs 設定は保持されます。", "無効なショートカット",
                "Todo リマインダー", "クイックリマインダー（分）", "Markdown モード", "外部 Markdown 拡張子", "外部エディターで開く",
            ...
            ["ko"] = new(
                ...
                "설정", "동작", "모양", "쇼트컷", "적용", "닫기", "가장자리 및 창", "Todo 동작", "상단 바",
                "캡슐 사용", "가장자리 캡슐 사용", "애니메이션 사용", "도구 설명 표시", "완료 항목을 아래로", "완료 Todo 자동 정리",
                "상단 바에 새 Todo", "상단 바에 새 Note", "상단 바에 외부 편집기", "테마", "색상", "전체 배율", "텍스트", "Todo 크기", "Note 크기",
                "제목 크기", "캡슐 크기", "굵게", "창 투명도", "비활성 메모 흐리게", "비활성 투명도", "숫자 키패드 구분",
                "Avalonia에서 구현된 일반 단축키만 편집합니다. 가장자리/Labs 설정은 유지됩니다.", "잘못된 쇼트컷",
                "Todo 알림", "빠른 알림(분)", "Markdown 모드", "외부 Markdown 확장자", "외부 편집기로 열기",
...
internal sealed record TextSet(
    ...
    string OpenExternalEditor,
    string OpenLinkedTarget,
    string LinkPaper,
    string LinkFile,
    string LinkFolder,
    string ClearLink,
